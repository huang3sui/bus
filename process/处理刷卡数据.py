...
for i in ['线路6', '线路11']:
    ...
    if i == '线路6':
        line_6_24hour = line_df[line_df['line_name'] == i]
        line_6_24hour = line_x_df.drop('line_name', axis=1)
        # line_6_df = line_x_df.drop('line_name', axis = 1)
    else:
        line_11_24hour = line_df[line_df['line_name'] == i]
        line_11_24hour = line_x_df.drop('line_name', axis=1)
        # line_11_df = line_11_24hour[line_6_24hour['deal_time'].dt.hour.isin(np.arange(6, 22))]
        # line_11_df = line_x_df.drop('line_name', axis = 1)

# 分开存储
line_6_24hour.to_csv('E:\公交客流预测\data\\6路公交24h刷卡记录.csv', encoding = 'gbk', index = False)
line_11_24hour.to_csv('E:\公交客流预测\data\\11路公交24h刷卡记录.csv', encoding = 'gbk', index = False)
